...
const Update1 = ({ isOpen, onClose, rateValue, onRateChange }) => {
  ...
  const handleSetNewRate = async () => {
    setIsLoading(true);
    try {
      const apiUrl = `https://api.tuma-app.com/api/treasury/new-interbank-rate?baseCurrency=GBP&targetCurrency=KES&interbankRate=${rateValue}&markUp=0&weightedAverage=0`;
